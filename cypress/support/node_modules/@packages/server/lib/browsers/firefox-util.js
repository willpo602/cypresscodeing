...
async function connectToNewSpec(options, automation, browserCriClient) {
    ...
    await cdp_automation_1.CdpAutomation.create(pageCriClient.send, pageCriClient.on, browserCriClient.resetBrowserTargets, automation, options.experimentalSessionAndOrigin);
    await options.onInitializeNewBrowserTab();
    debug(`firefox: navigating to ${options.url}`);
    await navigateToUrl(options.url);
}
async function setupRemote(remotePort, automation, onError, options) {
    const browserCriClient = await browser_cri_client_1.BrowserCriClient.create(remotePort, 'Firefox', onError);
    const pageCriClient = await browserCriClient.attachToTargetUrl('about:blank');
    await cdp_automation_1.CdpAutomation.create(pageCriClient.send, pageCriClient.on, browserCriClient.resetBrowserTargets, automation, options.experimentalSessionAndOrigin);
...
exports.default = {
    ...
    setup({ automation, extensions, onError, url, marionettePort, foxdriverPort, remotePort, options, }) {
